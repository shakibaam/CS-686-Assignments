...
class NeuralNetwork():
    ...
    def forward_pass(self, X) -> (List[np.ndarray], List[np.ndarray]):
        ...
        z = X
        for i in range(self.n_layers):
            # Add bias terms to the input of this layer
            x = np.concatenate([np.ones((z.shape[0], 1)), z], axis=1)

            a_i = np.dot(x,self.W[i])
            A_vals.append(a_i)

            z = self.activations[i].value(a_i)
            Z_vals.append(z)
    ...
    def backward_pass(self, A_vals, dLdyhat) -> List[np.ndarray]:
        ...
        for i in range(self.n_layers-2,-1,-1):
            weights_without_bias = self.W[i + 1][1:, :] 

            delta = np.dot(delta, weights_without_bias.T) * self.activations[i].derivative(A_vals[i]) 
            deltas.append(delta)

        deltas.reverse()  # Have deltas in forward order   
    ...
    def update_weights(self, X, Z_vals, deltas) -> List[np.ndarray]:
        ...
        for i in range(self.n_layers):
            if(i==0):
                # Add bias term to the input of this layer
                x_with_bias = np.concatenate([np.ones((X.shape[0], 1)), X], axis=1)
                gradient = np.dot(np.transpose(x_with_bias),deltas[i])
            else:
                z_with_bias = np.concatenate([np.ones((Z_vals[i - 1].shape[0], 1)), Z_vals[i - 1]], axis=1)
                gradient = np.dot(np.transpose(z_with_bias),deltas[i])
